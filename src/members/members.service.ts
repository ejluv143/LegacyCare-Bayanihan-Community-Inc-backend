...
function createActivationExpirationDate():
  Date {
...
export class MembersService {
  ...
  async createMember(
    ...
    const normalized =
      this.normalizeCreateInput(input);
    ...
    const sponsor =
      await this.findEligibleSponsor(
    ...
    const activationExpiresAt =
      createActivationExpirationDate();

    const membershipId =
      await this.generateUniqueMembershipId();
...
updatedAt:
  ...
  private async findEligibleSponsor(
    sponsorReferralCode: string,
  ) {
    const sponsor =
  ...
  private async ensureUniqueMemberFields(
    ...
    if (
      ...
      throw new ConflictException(
        "The CP number is already in use.",
      );
    }
  ...
  private async generateUniqueMembershipId():
    Promise<string> {
    for (
      let attempt = 0;
      attempt <
      ...
      const membershipId =
        `${MEMBERSHIP_ID_PREFIX}-${dateSegment}-${randomSegment}`;
  ...
  private async generateUniqueReferralCode():
    ...
    for (
      ...
      const referralCode =
        `${REFERRAL_CODE_PREFIX}-${generateRandomCode(8)}`;
  ...
  private handlePrismaCreateError(
    error: unknown,
  ): void {
    if (
      !(

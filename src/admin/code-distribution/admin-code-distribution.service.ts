...
export class AdminCodeDistributionService {
  ...
  async getActiveSatellites() {
    const satellites =
      await this.prisma.satellite.findMany({
        where: {
  ...
  async sendCodes(
    ...
    if (
      dto.codeIds.length === 0
    ) {
      throw new BadRequestException(
        "No generated codes selected.",
      );
    }

    const satellite =
      await this.prisma.satellite.findUnique({
        where: {
          id: dto.satelliteId,
        },
    ...
    if (
      ...
      throw new ConflictException(
        "Satellite is not active.",
      );
    }

    const codes =
      await this.prisma.generatedCode.findMany({
        where: {
          id: {
            in: dto.codeIds,
          },
    ...
    if (
      ...
      throw new NotFoundException(
        "Some generated codes were not found.",
      );
    }
    ...
    if (
      ...
      throw new ConflictException(
        "Some generated codes are not available.",
      );
    }
    ...
    return {
      ...
      message: `${assignedCodes.length} code${
        assignedCodes.length === 1
          ? ""
          : "s"
      } successfully sent to ${satellite.satelliteName}.`,
